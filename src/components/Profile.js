...
const Profile = (props) => {
  return (
    <>
      <article className="bg-white p-5 rounded shadow shadow-white">
        ...
        <div className="flex items-center justify-between flex-wrap mt-5 overflow-scroll">
          <ul className="text-xs flex items-center justify-start">
            <li className="py-1 px-2 text-white bg-blue-500 opacity-75 rounded-lg shadow inline-block mr-2">
              {props.language ?? "-"}
            </li>

            {props?.topics?.map((topic, index) => (
              <div key={index}>
                <li className="py-1 px-2 text-white bg-red-500 opacity-75 rounded-lg shadow inline-block mr-2">
                  {topic}
                </li>
              </div>
            ))}
